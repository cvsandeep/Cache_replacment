...
"information is generated.\n"
  ...
  opt_reg_string(odb, "-cache:dl1",
		 ...
		 &cache_dl1_opt, "dl1:256:32:1:3:l", /* print */TRUE, NULL);
  opt_reg_note(odb,
...
"                -dtlb dtlb:128:4096:32:r\n"
  ...
  opt_reg_string(odb, "-cache:dl2",
		 ...
		 &cache_dl2_opt, "ul2:1024:64:4:3:l", /* print */TRUE, NULL);
  opt_reg_string(odb, "-cache:il1",
		 "l1 inst cache config, i.e., {<config>|dl1|dl2|none}",
		 &cache_il1_opt, "il1:256:32:1:3:l", /* print */TRUE, NULL);
  opt_reg_note(odb,
...
"      -cache:dl1 ul1:256:32:1:l -cache:dl2 ul2:1024:64:2:l\n"
  ...
  opt_reg_string(odb, "-tlb:itlb",
		 ...
		 &itlb_opt, "itlb:16:4096:4:3:l", /* print */TRUE, NULL);
  opt_reg_string(odb, "-tlb:dtlb",
		 "data TLB config, i.e., {<config>|none}",
		 &dtlb_opt, "dtlb:32:4096:4:3:l", /* print */TRUE, NULL);
  ...
  if (!mystricmp(cache_dl1_opt, "none"))
  ...
  else /* dl1 is defined */
    {
      if (sscanf(cache_dl1_opt, "%[^:]:%d:%d:%d:%d:%c",
		 name, &nsets, &bsize, &assoc, &rrpv_width, &c) != 6)
	fatal("bad l1 D-cache parms: <name>:<nsets>:<bsize>:<assoc>:<rrpv_width>:<repl>");
      cache_dl1 = cache_create(name, nsets, bsize, /* balloc */FALSE,
			       /* usize */0, assoc, rrpv_width, cache_char2policy(c),
  ...
  if (!mystricmp(cache_il1_opt, "none"))
  ...
  else /* il1 is defined */
    {
      if (sscanf(cache_il1_opt, "%[^:]:%d:%d:%d:%d:%c",
		 name, &nsets, &bsize, &assoc, &rrpv_width, &c) != 6)
	fatal("bad l1 I-cache parms: <name>:<nsets>:<bsize>:<assoc>:<rrpv_width>:<repl>");
  ...
  if (!mystricmp(itlb_opt, "none"))
  ...
  else
    {
      if (sscanf(itlb_opt, "%[^:]:%d:%d:%d:%d:%c",
		 name, &nsets, &bsize, &assoc, &rrpv_width, &c) != 6)
	fatal("bad TLB parms: <name>:<nsets>:<page_size>:<assoc>:<rrpv_width>:<repl>");
      itlb = cache_create(name, nsets, bsize, /* balloc */FALSE,
			  /* usize */sizeof(md_addr_t), assoc, rrpv_width, 
